...
Run from the project root. This parses all source files, builds the knowledge graph, writes it to `.codragraph/`, and generates AGENTS.md / AGENTS.md context files.
...
**When to run:** First time in a project, after major code changes, or when `codragraph://repo/{name}/context` reports the index is stale. In Codex, a PostToolUse hook runs `analyze` automatically after `git commit` and `git merge`, preserving embeddings if previously generated.
